...
# Derived from E11's verified result files (e11_summary.json, pretest.json), NOT from
# copied source defaults. This is the guard against the stale-relay-default mistake in
# mistake/mistake-2026-09-13-e11-stale-relay-default-caught.md.
MODEL = os.environ.get("E12_MODEL", "nousresearch/hermes-4-405b")
API = os.environ.get("E12_API", "https://openrouter.ai/api/v1/chat/completions")
API_KEY_ENV = os.environ.get("E12_KEY_ENV", "OPENROUTER_API_KEY")
VERSION = "e12-partitioned-dags-v2"
ADAPTER_KEY = b"e11-development-adapter-key"  # same packet adapter as E11
ARMS = ("solo_planner", "swarm_proposal", "swarm_partitioned")
SEEDS = list(range(101, 121))
RULES = ("acyclicity", "reachability", "contradiction_cascade", "resource_consistency", "verifier_feasibility")
# E12 budget cap is $1.00 (program has ~$6 left of $10). Three arms: $0.33 each keeps
# total below cap with headroom; E11 used $0.65/arm under a $2 cap.
MAX_ARM_COST = 0.33
...
class Pilot:
    ...
    def call(self, key: str, prompt: str) -> Tuple[str, Dict[str, Any]]:
        ...
        if not api_key:
            raise RuntimeError(API_KEY_ENV + " not set")
        payload = {
            "model": MODEL,
        ...
        for attempt in range(5):
            try:
                req = request.Request(API, data=data, headers={
                    ...
                    "X-Title": "antelligence-e12-partitioned-dags",
                })
...
Return ONLY one JSON object, no markdown, with this exact top-level shape: {"goal":"node-id","nodes":[{"id":"local-id","parents":["local-id"],"resource":"resource-name","evidence":[{"source_id":"fixture","revision":0}],"action":{"op":"reserve","sample":"<your-actual-sample-name>","slot":"slot-0","revision":0}}]}. Build a complete executable sub-DAG for your sample: a reserve node followed by a place node that depends on the reserve. A child sample's reserve depends on all prerequisite sample place nodes; since you only know your own prerequisites, express them as parents if they appear in your slice, otherwise chain off the most recent node. Use local parent references only. Use the ACTUAL sample name from your private slice (the value of the "my_sample" key in the fixture json — e.g. "sample-0", "sample-1", or "sample-2") as the "sample" field in every action. Never include evaluator fields or prose.
...
def proposals_for(task: Dict[str, Any], seed: int, arm: str, pilot: Any) -> Tuple[List[Dict[str, Any]], int, int]:
    ...
    for agent in agents:
        ...
        text, _usage = pilot.call(key, prompt_for(task, seed, arm, agent))
        calls += 1
        try:
...
def run_arm(arm: str, seeds: List[int]) -> Dict[str, Any]:
    ...
    for seed in seeds:
        ...
        try:
            ...
            raise
        admitted = admit(task, proposals)
        second = admit(task, proposals)
    ...
    result = {
        "experiment": "e12-partitioned-dags",
        "model": MODEL,
        "provider": "openrouter",
        "api": API,
        "api_key_env": API_KEY_ENV,
        "arm": arm,
        "config": {"seeds": seeds, "temperature": 0.0, "max_tokens": 1800, "max_arm_cost": MAX_ARM_COST, "rules": list(RULES)},
        "runs": runs,
...
def run_selftest() -> int:
    ...
    payload = {"experiment": "e12-partitioned-dags", "offline": True, "policy": "ScriptedDAGPolicy", "pass": passed, "rule_cases": details, "matrix_runs": matrix, "run_count": len(matrix)}
...
def run_pretest() -> int:
    """Cheap pre-test gate: 3 seeds, swarm_partitioned arm only (the new arm).

    Confirms that the partitioned prompt produces parseable proposals before the matrix
    spends any of the $1.00 cap. solo_planner and swarm_proposal are NOT re-pretested;
    they are anchors that reproduce E11.
    """
    pilot = Pilot("pretest", max_cost=0.05)
    rows: List[Dict[str, Any]] = []
    for seed in SEEDS[:3]:
        task = fixture_task(seed)
        for agent in ["agent-0", "agent-1", "agent-2"]:
            key = "|".join((VERSION, "pretest", str(seed), "swarm_partitioned", agent, "dag"))
            try:
                text, _usage = pilot.call(key, prompt_for(task, seed, "swarm_partitioned", agent))
                try:
                    parsed = parse_proposal(text)
                    rows.append({"seed": seed, "agent": agent, "parseable": True, "nodes": len(parsed["nodes"])})
                except ValueError as exc:
                    rows.append({"seed": seed, "agent": agent, "parseable": False, "error": str(exc)})
            except Exception as exc:
                rows.append({"seed": seed, "agent": agent, "parseable": False, "error": repr(exc)})
    # Pass requires ALL 9 (3 seeds × 3 agents) parseable.
    passed = len(rows) == 9 and all(row["parseable"] for row in rows)
    result = {
        "experiment": "e12-partitioned-dags",
        "gate": "pretest",
        "model": MODEL,
...
def aggregate() -> int:
    ...
    summary: Dict[str, Any] = {"experiment": "e12-partitioned-dags", "seeds": SEEDS, "arms": {}, "determinism_all_pass": True, "pretest_cost_usd": pretest_cost}
    ...
    summary["total_cost_usd"] = summary["matrix_cost_usd"] + pretest_cost
    summary["budget_cap_pass"] = summary["total_cost_usd"] <= 1.0
    write_json(OUT / "e12_summary.json", summary)
    lines = ["# E12 — partitioned-view swarm task DAGs", "",
    ...
    lines += ["",
              "Total matrix cost: `${:.6f}`; pre-test cost: `${:.6f}`; total: `${:.6f}` (cap pass: **{}**).".format(
                  summary["matrix_cost_usd"], pretest_cost, summary["total_cost_usd"], summary["budget_cap_pass"]),
              "",
    ...
    print(json.dumps({"summary": str(OUT / "e12_summary.json"), "total_cost_usd": summary["total_cost_usd"], "determinism_all_pass": summary["determinism_all_pass"], "budget_cap_pass": summary["budget_cap_pass"]}, sort_keys=True))
