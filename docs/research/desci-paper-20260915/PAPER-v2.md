...
**Draft status:** v2 revision — E13 second headline; E12/E14 coverage claims marked confounded (pending E15); based only on existing local experiment reports and the review snapshot.  
**Authors:** [TODO: verify]  
...
The negative results define the boundary. Communication has no **isolated demonstrated throughput value**: its contribution is defensive admission, and E10's dose-response shows search efficiency rising with memory scope rather than with privacy. E13's communication arm was numerically lower than shared memory, but that arm was not an isolated preregistered communication contrast. E11 adds a deterministic admission result but also shows that independent swarm proposals cost approximately 2.7x more tokens per accepted node without a material success lift, and a same-harness rerun measured proposal reproducibility at 19 of 20 seeds even at temperature 0. Full-spatial control remains model-bound, with a capability floor in 2 of 3 tested model conditions. The partitioned-DAG coverage experiments (E12 and E14) are marked confounded: their 0/20 outcomes were structurally guaranteed by under-provisioned partitioning (three agents against a four-sample fixture, with the terminal sample never proposed), so only their zero-redundancy results are retained, and E15 re-runs them with corrected provisioning. Antelligence therefore should be presented as evidence and safety infrastructure for swarm experiments—not as a general intelligence multiplier, a clinical system, or a substitute for capable action policies.
...
* E12 partitioned DAGs (coverage confounded; redundancy stands): `/Volumes/WD_BLACK/antelligence-experiments-20260911/e12-partitioned-dags/REPORT.md` and `out/e12_summary.json`
* E14 merge/repair (coverage confounded; redundancy stands): `/Volumes/WD_BLACK/antelligence-experiments-20260911/e14-merge-repair/REPORT.md` and `out/e14_summary.json`
