...
**Draft status:** v1 revision, based only on existing local experiment reports and the review snapshot.  
**Authors:** [TODO: verify]  
...
E13 changes the scope of the headline, but not its evidentiary boundary. It is a cross-model replication on a second, structurally different task family, not an exact same-model reproduction of the E7/E9 anchors; the new family adds temporal delivery prerequisites and a 120-step budget. The paper makes three narrower contributions:
...
3. It records conditional and negative results as first-class results, including E10's privacy-dependent communication effect, E11's costly redundant swarm proposals, and the persistence of a full-spatial model floor.
...
memory-plus-communication arm reduced sweep moves by 15.27% (3057→2590), with 17 wins,
2 losses, and 1 tie; p=0.000364. All arms failed only on seed 115, delivering 2/3 foods.
Communication operated in the comm arm (`transport_events=380`), but this arm-level observation
does not identify an independent communication effect relative to shared memory. The pre-registered
bar—at least 5% reduction and p<0.05 for hive memory—was cleared. E13 therefore supports a
bounded generality claim across two task families, with the explicit cross-model caveat.

Artifacts: `/Volumes/WD_BLACK/antelligence-experiments-20260911/e13-chain/out/e13_summary.json`
and the three per-arm result files.

### 4.8 E14 deterministic merge/repair over partitioned DAGs

E14 tested whether a model-free merge layer could repair the coverage loss caused by E12's partitioned proposers. The merge unioned the three partitioned proposals, namespaced local node IDs, rewired each reserve to the place nodes of its declared prerequisites, and passed the result through the unchanged deterministic admission gate. This run used `z-ai/glm-5.3-flash` through the Nous Portal, so the E12 anchor comparison is cross-model; the predeclared anchor values were not reproduced exactly.
...
The predeclared verdict is **REFUTED**: deterministic merge preserved zero redundancy but did not preserve coverage—merged execution remained 0/20 versus solo's 9/20—and merged token cost was 3.72x the new solo arm. Admission was byte-identical across both runs for every seed. This is a cross-model replication of the boundary, not evidence that the E12 anchor numbers are model-invariant. Artifacts: `/Volumes/WD_BLACK/antelligence-experiments-20260911/e14-merge-repair/out/e14_summary.json` and the three per-arm result files.
...
| Coordination and redundant-work failure | Shared food sightings and target retrieval in the binary harness | E7v6, E9-binary, and E13: memory reduced sweep moves in two task families; E13 3057→2655, p=0.000656 | Comparisons are cross-model across families and remain bounded to the tested protocols |
...
The current evidence supports a bounded architectural claim. Shared evidence memory paired with a model-free verifier makes a capability-matched LLM swarm about 16% more search-efficient in the E7v6 binary harness, and E13 extends the reduction to 13.15% in a chain-prioritized second task family (p=0.000656). The E7/E9 anchors replicate across hermes-4-405b and grok-4-fast, while E13 is a separate cross-model Nous run; together they support bounded cross-family generality, not model-independent universal performance. The broader mechanism suite shows why the architecture can matter: memory plus verification is cheaper than always re-querying under silent drift, unsafe stale-memory actions are blocked, the verifier classifies all tested cases, and bounded transport rejects attacks that naive message passing admits.

The negative results define the boundary. Communication has no **isolated demonstrated throughput value**: its contribution is defensive admission, and E10's dose-response shows search efficiency rising with memory scope rather than with privacy. E13's communication arm was numerically lower than shared memory, but that arm was not an isolated preregistered communication contrast. E11 adds a deterministic admission result but also shows that independent swarm proposals cost approximately 2.7x more tokens per accepted node without a material success lift, and a same-harness rerun measured proposal reproducibility at 19 of 20 seeds even at temperature 0. Full-spatial control remains model-bound, with a capability floor in 2 of 3 tested model conditions. E14 adds a cross-model negative boundary: partitioning plus deterministic prerequisite rewiring preserved zero redundancy but still produced 0/20 execution successes, so the tested merge layer did not restore coverage. Antelligence therefore should be presented as evidence and safety infrastructure for swarm experiments—not as a general intelligence multiplier, a clinical system, or a substitute for capable action policies.
